...
interface GenerarAutoPrimeraAudienciaProps {
  caseId: string;
...
export function GenerarAutoPrimeraAudiencia({
  caseId,
  hearingId,
}: GenerarAutoPrimeraAudienciaProps) {
  ...
  const [opts, setOpts] = useState<AutoPrimeraAudienciaOpciones | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const initial: AutoPrimeraAudienciaOpciones = {
          numero_auto: "",
  ...
  // ── Helpers de estado ───────────────────────────────────────────────────
  function patch(p: Partial<AutoPrimeraAudienciaOpciones>) {
  ...
  async function handleGenerar() {
    ...
    const opciones: AutoPrimeraAudienciaOpciones = { ...opts, considerandos };
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-10 text-center">
        ...
        <p className="text-xs text-gray-400 mt-1">
          Programa una audiencia antes de generar el auto de primera audiencia.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6 mt-6">
      ...
      <section className="bg-white rounded-xl shadow-sm border border-gray-100 px-6 py-5">
        ...
        <label className="flex items-center gap-2.5 text-sm text-gray-700 cursor-pointer">
          <input
            type="checkbox"
            checked={opts.hubo_reparos}
            onChange={(e) => patch({ hubo_reparos: e.target.checked })}
            className="h-4 w-4 rounded border-gray-300 text-[#1B4F9B] focus:ring-[#1B4F9B]"
          />
          <span>Se presentaron reparos al auto admisorio</span>
        </label>

        {opts.hubo_reparos && (
          <div className="mt-4 space-y-4">
            <div>
              <label className={labelCls}>¿Qué reparos se plantearon?</label>
              <textarea
                value={opts.reparos_descripcion}
                onChange={(e) => patch({ reparos_descripcion: e.target.value })}
                rows={4}
                placeholder="Describe los reparos planteados al auto admisorio..."
                className={inputCls}
              />
            </div>
            <div>
              <label className={labelCls}>Decisión del operador</label>
              <textarea
                value={opts.decision_operador}
                onChange={(e) => patch({ decision_operador: e.target.value })}
                rows={4}
                placeholder="Describe la decisión del operador sobre los reparos..."
                className={inputCls}
              />
            </div>
          </div>

...
export interface PropuestaPagoEditorProps {
  acreedores: AcreedorFormData[];
  propuestaPago: PropuestaPagoClase[];
  condonacionesGlobales?: CondonacionesConfig;
  readOnly?: boolean;
  onChange: (patch: {
...
export function PropuestaPagoEditor({
  acreedores,
  propuestaPago,
  condonacionesGlobales,
  readOnly = false,
  onChange,
}: PropuestaPagoEditorProps) {
  const propuestas = propuestaPago;
  const condonaciones = condonacionesGlobales;
  ...
  function reemplazarClase(next: PropuestaPagoClase) {
    ...
    onChange({ propuesta_pago: [...otras, next] });
  }
  ...
  function actualizarCondonaciones(patch: Partial<CondonacionesConfig>) {
    ...
    onChange({ condonaciones_globales: { ...base, ...patch } });
  }
  ...
  function asegurarCreditoClase14(clase: ClasePrelacion, ac: AcreedorConIndice): PropuestaPagoCredito {
    ...
    return {
      ...defaults,
      cronograma,
      redaccion_narrativa: generarRedaccionCredito({ ...defaults, cronograma }, ac, clase),
    };
  }
  ...
  function actualizarClase5(
    ...
    const next: PropuestaPagoClase = { ...propActual, ...patch, clase_prelacion: "quinta" };
    ...
    if (next.prioridad_pequenos) {
      ...
      resultados = generarCronogramaPrioridadPequenos({
        pequenos, otros,
        numeroCuotasTotal: numeroCuotas, mCuotasPequenos: m,
        mesesGraciaCompartido: mesesGracia, tipoAmortizacion: tipoAmort,
      });
    } else {
    ...
    <section className="space-y-6">
      ...
      {clasesActivas.length === 0 && (
        <p className="text-sm text-gray-500">
          Agrega acreedores con su clase de prelación y tasa de interés para armar la propuesta.
        </p>
      )}

      {acreedoresSinTasa.length > 0 && (
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
          Debes diligenciar la <b>tasa de interés mensual</b> para los siguientes acreedores
...
function Num({
  label, val, onC, disabled,
}: { label: string; val: number; onC: (v: number) => void; disabled?: boolean }) {
  return (
    <div>
...
function Select({
  label, val, options, onC, disabled,
}: {
  label: string;
  val: string;
...
function Check({
  label, checked, onC, disabled,
}: { label: string; checked: boolean; onC: (v: boolean) => void; disabled?: boolean }) {
  return (
    <label className={`flex items-center gap-2 text-sm ${disabled ? "" : "cursor-pointer"}`}>
      <input
        type="checkbox"
        checked={checked}
...
function TablaCronograma({
  cronograma,
}: {
  cronograma: { cuota: number; mes_relativo: number; cuota_total: number; intereses: number; amortizacion: number; saldo: number }[];
}) {
...
function NarrativaEditable({
  value, onC, disabled,
}: { value: string; onC: (v: string) => void; disabled?: boolean }) {
  return (
    <details className="mt-2">
      ...
      <textarea
        ...
        onChange={(e) => onC(e.target.value)}
        disabled={disabled}
        className="w-full mt-2 border border-gray-300 rounded-lg px-3 py-2 text-xs font-sans disabled:bg-gray-50"
      />
    </details>
  );
}

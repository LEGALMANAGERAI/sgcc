// ============================================================
// Generador de Word del Auto de Primera Audiencia
// (firmeza del auto admisorio — Art. 550 C.G.P) de insolvencia.
//
// Reusa el ESQUELETO del auto de suspensión (membrete, ciudad/fecha,
// identificación, apertura, quórum, considerandos, tabla de acreencias,
// firmas, pie). Las diferencias están descritas en
// docs/autos/auto-primera-audiencia-estructura.md:
//   - Título "PRIMERA AUDIENCIA".
//   - Asunto "Auto de primera audiencia de negociación de deudas (Art. 550 C.G.P)".
//   - NO hay sección de continuación (no suspende).
//   - RESUELVE condicional según opciones.hubo_reparos (texto VERBATIM del .md).
//
// El texto fijo es VERBATIM del .md (no parafrasear). Las variables vienen
// de ResolvedAutoVars / AutoPrimeraAudienciaOpciones.
// ============================================================
import {
  Document,
  Packer,
...
import {
  ...
  type AutoPrimeraAudienciaOpciones,
  type QuorumFila,
} from "./types";
...
export async function generarAutoPrimeraAudiencia(
  vars: ResolvedAutoVars,
  opciones: AutoPrimeraAudienciaOpciones,
): Promise<Buffer> {
  ...
  // 3. Título — DIFERENCIA: "PRIMERA AUDIENCIA".
  children.push(
    new Paragraph({
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 240 },
      children: [new TextRun({ text: "PRIMERA AUDIENCIA", bold: true, size: 24, color: NAVY })],
    }),
  );

  // 4. Identificación — DIFERENCIA: asunto de primera audiencia (Art. 550).
  children.push(
    new Paragraph({
      spacing: { after: 60 },
      children: [
    ...
    new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: "Auto de primera audiencia de negociación de deudas (Art. 550 C.G.P)", size: BODY_SIZE }),
      ],
    }),
  );
  ...
  // DIFERENCIA: si hubo reparos y se describieron, van como un numeral de
  // considerandos justo antes del RESUELVE.
  if (opciones.hubo_reparos) {
    const reparos = (opciones.reparos_descripcion ?? "").trim();
    if (reparos.length > 0) {
      n += 1;
      children.push(bodyPara(`${n}) ${reparos}`, { spacingAfter: 160 }));
    }
  }
  ...
  // 10. III. RESUELVE (VERBATIM con huecos) — DIFERENCIA: condicional por reparos.
  children.push(bodyPara("III.    RESUELVE:", { bold: true, align: AlignmentType.LEFT, spacingAfter: 160 }));

  if (!opciones.hubo_reparos) {
    // Sin reparos → el auto admisorio queda en firme.
    const primero =
      `PRIMERO. DEJAR EN FIRME el Auto No. ${oBlank(caso.numero_auto_admisorio)} del ${oBlank(caso.fecha_auto_admisorio)}, ` +
      `mediante el cual se admitió el procedimiento de negociación de deudas, así como lo actuado hasta el momento, ` +
      `al no haberse presentado reparo jurídico alguno por los intervinientes.`;
    ...
    // Con reparos → decisión del operador + recurso de reposición.
    children.push(
      bodyPara(`PRIMERO. ${oBlank(opciones.decision_operador)}`, { spacingAfter: 160 }),
    );
    children.push(
      bodyPara(
    ...
    children.push(bodyPara("TERCERO. Los presentes quedan notificados en audiencia.", { spacingAfter: 480 }));
  }
  ...
  const doc = new Document({
    creator: "SIGECC",
    title: `Auto de primera audiencia — ${caso.radicado}`,
    sections: [
      {
        properties: {
